...
impl Agent for AgentServer {
    async fn cast_spell(
        ...
        // Add to chat history
        let agent_name = state.name.clone();
        state
            .chat_history
            .push(format!("Sorcerer: {}", spell.incantation));
        state
            .chat_history
            .push(format!("{}: [Message received]", agent_name));
        ...
        let result = SpellResponse {
            spell_id: spell.spell_id,
            result: "[Message received]".to_string(),
            success: true,
            error: String::new(),
        };
    ...
    async fn kill(&self, request: Request<KillRequest>) -> Result<Response<KillResponse>, Status> {
        ...
        info!("Agent being killed: {}", reason);
